...
import cz.zcu.luk.sspace.lsa.LSALoadStatsExpsStops;
 ...
 */
public class LSAMainLoadStatsExpsStops extends GenericMainLoadStatsStopwords {

    private BasisMapping<String, String> basis;

    private LSAMainLoadStatsExpsStops() {
    }
    ...
    protected void addExtraOptions(ArgOptions options) {
        ...
        options.addOption('B', "saveTermBasis",
                          ...
                          true, "FILE", "Optional");
        options.addOption('e', "expressionsInvestigated", "a file where each line is a " +
                "recognized expression for which a statistic is being done." +
                " No words' vectors are influenced", true, "FILE",
                "Program Options");
    }

    public static void main(String[] args) throws Exception {
        LSAMainLoadStatsExpsStops lsa = new LSAMainLoadStatsExpsStops();
        lsa.run(args);
    }
    
    protected SemanticSpaceLoadStats getSpace() {
        try {
            int dimensions = argOptions.getIntOption("dimensions", 300);
            TransformExtended transform = new LogEntropyTransformExtended(); // cannot be null, since the name is needed in GenericMainLoadStats..
            ...
            return new LSALoadStatsExpsStops(
                false, dimensions, transform, factorization, false, basis, expressions);
        } catch (IOException ioe) {
            throw new IOError(ioe);
        }
    }
