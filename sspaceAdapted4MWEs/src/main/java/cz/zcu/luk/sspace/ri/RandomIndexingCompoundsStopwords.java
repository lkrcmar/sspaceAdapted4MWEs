...
import cz.zcu.luk.sspace.common.WordTransformer;
 ...
 * cz.zcu.luk.sspace.ri.RandomIndexingCompoundsStopwords#RandomIndexing(java.util.Properties)} constructor.
 *
 ...
 *      class name of a {@link edu.ucla.sspace.index.PermutationFunction} instance that will be used
 ...
 * </dl> <p>
 *
 * This class implements {@link edu.ucla.sspace.common.Filterable}, which allows for fine-grained
 * control of which semantics are retained.  The {@link #setSemanticFilter(java.util.Set)}
 ...
 * #processDocument(java.io.BufferedReader) processDocument}.  At any given point in
 ...
 *
 * The {@link #processSpace(java.util.Properties) processSpace} method does nothing for
 ...
 */
public class RandomIndexingCompoundsStopwords implements SemanticSpace, Filterable {

    public static final String RI_SSPACE_NAME
        = "xRIC"; //"random-indexing";
     ...
     * edu.ucla.sspace.index.PermutationFunction} if using permutations is enabled.
     */
     ...
     */
    public RandomIndexingCompoundsStopwords() {
     ...
     */
   public RandomIndexingCompoundsStopwords(Properties properties) {
        this(properties, null);
    }

    public RandomIndexingCompoundsStopwords(Properties properties, Set<String> compounds) {
   ...
   /**
    ...
    public String getSpaceName() {
        return RI_SSPACE_NAME + "_L" + vectorLength + "_W" + windowSize + "_P"
            + ((usePermutations)
    ...
    /**
     * Returns an unmodifiable view on the token to {@link edu.ucla.sspace.vector.IntegerVector}
    ...
    public void processDocument(BufferedReader document) throws IOException {
        ...
        while (!nextWords.isEmpty()) {
            ...
            focusWord = WordTransformer.getWord(possibleWord);
            ...
            // semantics around
            // LK change - brackets added, before the meaning of EMPTY_TOKEN was counted..
            boolean calculateSemantics =
                (semanticFilter.isEmpty() || semanticFilter.contains(focusWord))
                && !focusWord.equals(IteratorFactoryStopwords.EMPTY_TOKEN);
            ...
            if (calculateSemantics) {
                ...
                for (String possibleW : prevWords) {
                    ...
                    // into account.
                    String word = WordTransformer.getWord(possibleW);
                ...
                permutations = 1;
                for (String possibleW : nextWords) {
                    // Skip the addition of any words that are excluded from the
                    // filter set.  Note that by doing the exclusion here, we
                    // ensure that the token stream maintains its existing
                    // ordering, which is necessary when permutations are taken
            ...
            if (compounds != null) {
                ...
                String possibleCompoundTrigram = "NOT_A_COMPOUND";
                String possibleCompoundBigram = "NOT_A_COMPOUND";
                if (prevWordsArr.length > 1) {
                    prevPrevWord = prevWordsArr[prevWordsArr.length - 2];
                    prevWord = prevWordsArr[prevWordsArr.length - 1];
                    possibleCompoundTrigram = WordTransformer.getWord(prevPrevWord) + " " +
                            WordTransformer.getRealWord(prevWord) + " " + WordTransformer.getWord(focusWord);
                }
                if (prevWordsArr.length > 0) {
                    prevWord = prevWordsArr[prevWordsArr.length - 1];
                    possibleCompoundBigram = WordTransformer.getWord(prevWord) + " " +
                            WordTransformer.getWord(focusWord);
                }
                ...
                if (compound != null) {
                    IntegerVector focusMeaning = getSemanticVector(compound);
                    ...
                    for (String possibleW : prevWordsList) {
                        // Skip the addition of any words that are excluded from the
                        // filter set.  Note that by doing the exclusion here, we
                        // ensure that the token stream maintains its existing
                        // ordering, which is necessary when permutations are taken
                        ...
                        add(focusMeaning, iv);
                        //System.out.println("adding prev: " + compound + ", "+ word);
                    }
                    ...
                    for (String possibleW : nextWords) {
                        ...
                        add(focusMeaning, iv);
                        //System.out.println("adding after: " + compound + ", "+ word);
                    }
                }
            }
            ...
            if (prevWords.size() > windowSize) {
                // LK changed.. from.. prevWords.remove();
                removedOld = removed;
    ...
    */
    ...
    /**
     * Assigns the token to {@link edu.ucla.sspace.vector.IntegerVector} mapping to be used by this
