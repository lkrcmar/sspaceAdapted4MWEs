 ...
 * streams of tokens such as {@link java.io.BufferedReader} instances.  This class
 ...
 *      edu.ucla.sspace.text.TokenFilter} that should be applied to all token streams.<p>
 *
 ...
 *      {@link edu.ucla.sspace.text.Stemmer} on all the tokens returned by iterators of this class.
 ...
 * if filtering is enabled.  The {@link #tokenize(java.io.BufferedReader) tokenize}
 ...
 * #tokenizeOrdered(java.io.BufferedReader) tokenizeOrdered} method should be used
 ...
 *
 * @see edu.ucla.sspace.text.WordIterator
 * @see edu.ucla.sspace.text.TokenFilter
 * @see edu.ucla.sspace.text.CompoundWordIterator
 */
public class IteratorFactoryStopwords {

    /**
     * The signifier that stands in place of a token has been removed from an
     * iterator's token stream by means of a {@link edu.ucla.sspace.text.TokenFilter}.  Tokens
     * returned by {@link #tokenizeOrdered(java.io.BufferedReader) tokenizeOrdered} may
    ...
    // LK added
    public static final String STOPWORD_FLAG = "  ";
    ...
    /**
     * Specifies the {@link edu.ucla.sspace.text.TokenFilter} to apply to all iterators generated by
     * this factory
     */
    ...
    /**
     * Specifies the {@link edu.ucla.sspace.text.Stemmer} to use on tokens.  If not set, no stemming
     * is done.
     */
     ...
     * {@link cz.zcu.luk.sspace.text.IteratorFactoryStopwords}.
     */
    ...
    static {
        ITERATOR_FACTORY_PROPERTIES.add(
                IteratorFactoryStopwords.TOKEN_FILTER_PROPERTY);
        ITERATOR_FACTORY_PROPERTIES.add(
                IteratorFactoryStopwords.STEMMER_PROPERTY);
        ITERATOR_FACTORY_PROPERTIES.add(
                IteratorFactoryStopwords.COMPOUND_TOKENS_FILE_PROPERTY);
        ITERATOR_FACTORY_PROPERTIES.add(
                IteratorFactoryStopwords.TOKEN_REPLACEMENT_FILE_PROPERTY);
        ITERATOR_FACTORY_PROPERTIES.add(
                IteratorFactoryStopwords.TOKEN_COUNT_LIMIT_PROPERTY);
    }
    ...
    /**
     * The {@link edu.ucla.sspace.util.ResourceFinder} used to locate the file-based resources used
     ...
     * edu.ucla.sspace.text.CompoundWordIterator} doing the tokenizing if compound word support is
     ...
     * Uninstantiable
     */
    private IteratorFactoryStopwords() { }
    ...
    /**
     * Sets the {@link edu.ucla.sspace.util.ResourceFinder} used by the iterator factory to locate
     * its file-based resources when configuring the tokenization.  This method
     * should be set prior to calling {@link #setProperties(java.util.Properties)
     ...
     * those that were removed by any configured {@link edu.ucla.sspace.text.TokenFilter}.
     *
    ...
    private static Iterator<String> getBaseIterator(BufferedReader reader,
        ...
        if (filter != null) {
            finalIterator = (keepOrdering)
                ? new OrderPreservingFilteredIteratorStopwords(finalIterator, filter)
