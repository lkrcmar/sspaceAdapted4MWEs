...
import cz.zcu.luk.sspace.common.SimpleHistogram;
 ...
 * 
 * LK change - see in code - does not count PMI and other stats correctly
 */
public class BigramExtractorChangedOpt {
    ...
    private static final Logger LOGGER =
        Logger.getLogger(BigramExtractorChangedOpt.class.getName());
     ...
     */
    private long numBigramsInCorpus;

    /**
     * Creates a new bigram extractor
     */
//    public BigramExtractorChangedOpt() {
//        this(1000);
//    }
     ...
     * number of bigrams
     */
    public BigramExtractorChangedOpt(int expectedNumBigrams, String wordsFileName) {
        ...
        // LK added
        wordsInExps = new TrieMap<String>();
        List<String> loadedWords = Common.readLinesFromFile(wordsFileName);
        for (String oneWord : loadedWords) {
            wordsInExps.put(oneWord, "nothing");
        }
        // LK added
        ...
        try {
            while ((line = text.readLine()) != null) {
                if (deleteDeterminers) {
                	throw new UnsupportedOperationException("do not know why to replace determiners -> not implemented");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_1, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_2, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_3, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_4, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_5, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_6, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_7, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_8, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.MULTIPLE_DET_9, " MuLtIpLeDeTeRmInEr_XX ");
//                    line = line.replaceAll(Constants.DET_A_WITH_SPACES, " ");
//                    line = line.replaceAll(Constants.DET_AN_WITH_SPACES, " ");
//                    line = line.replaceAll(Constants.DET_THE_WITH_SPACES, " ");
    ...
    public void processInWindow(Iterator<String> text, int windowSize) {
    	...
    	while (text.hasNext()) {
			...
			for (int j = 1; j < windowSize; j++) {
				if (!(excludeToken(window[0]) || excludeToken(window[j]))) { // or - is not counting PMI correctly!
...
//    }
    ...
    public void printBigrams(PrintWriter output, 
                             SignificanceTest test, int minOccurrencePerToken, boolean retainJJNNVVOnly) {
        ...
        LOGGER.info("Number of stored distinct bigrams: " + bigramCounts.size());
        LOGGER.info("Count of all processed bigrams: " + numBigramsInCorpus);

        SimpleHistogram histogram = new SimpleHistogram(5000);
        for (Map.Entry<Long,Number> e : bigramCounts.entrySet()) {
            ...
            if (retainJJNNVVOnly) {
                if (excludeNonJJNNVV(indexToToken[firstTokenIndex]) || excludeNonJJNNVV(indexToToken[secondTokenIndex])) {
                    continue;
                }
            }
            ...
            histogram.addOccurrenceOfValue(bigramCount);
...
//                continue;
            ...
            long[] contingencyTable = getContingencyTable(t1, t2, bigramCount);
...
//            }
    ...
    private boolean excludeNonJJNNVV(String token) {
        return (!(token.endsWith("NN") || token.endsWith("VV") || token.endsWith("JJ")));
    }
     ...
     */
    private double getScore(long[] contingencyTable, SignificanceTest test) {
        switch (test) {
        case PMI:
     ...
     * table
     */
    private double pmi(long[] contingencyTable) {
        // Rename for short-hand convenience
        long[] t = contingencyTable;
     ...
     */
    private double chiSq(long[] contingencyTable) {
        // Rename for short-hand convenience
        long[] t = contingencyTable;
        long col1sum = t[0] + t[2];
        long col2sum = t[1] + t[3];
        long row1sum = t[0] + t[1];
        long row2sum = t[2] + t[3];
     ...
     */
    private double logLikelihood(long[] contingencyTable) {
        // Rename for short-hand convenience
        long[] t = contingencyTable;
        long col1sum = t[0] + t[2];
        long col2sum = t[1] + t[3];
     ...
     */
    private long[] getContingencyTable(TokenStats leftTokenStats,
                                      TokenStats rightTokenStats,
                                      int bigramCount) {
        long leftTokenOnLeftInAnyBigram = leftTokenStats.leftCount;
        long rightTokenOnRightInAnyBigram = rightTokenStats.rightCount;
        ...
        long b = rightTokenOnRightInAnyBigram - a;
        // The number of times the left token appeared as the left token in some
        // other bigram without the current right token
        long c = leftTokenOnLeftInAnyBigram - a;
        // The total number of bigrams in which neither the current left or
        // right token appeared
        long d = (numBigramsInCorpus - (b + c + a));
...
//
//        try {
//            BigramExtractorChangedOpt be = new BigramExtractorChangedOpt(1000000); // 1M
