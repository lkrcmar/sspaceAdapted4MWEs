 ...
 * instances.
 *
 * @see edu.ucla.sspace.matrix.Matrix
 * @see edu.ucla.sspace.matrix.MatrixIO
 */
public class Matrices {
    ...
    /**
     * Returns a {@link edu.ucla.sspace.matrix.Matrix} from a list of {@code DoubleVector}s.  The
    ...
    /**
     * Returns a {@link edu.ucla.sspace.matrix.SparseMatrix} from a list of {@code
    ...
    /**
     * Returns a {@link edu.ucla.sspace.matrix.MatrixBuilder} in the default format of the fastest
     * available {@link edu.ucla.sspace.matrix.SVD.Algorithm SVD algorithm}.
     *
     * @return a matrix builder to be used in creating a matrix for use with the
     *         {@link edu.ucla.sspace.matrix.SVD} class
     */
    ...
    /**
     * Returns a {@link edu.ucla.sspace.matrix.MatrixBuilder} in the default format of the fastest
     * available {@link edu.ucla.sspace.matrix.SVD.Algorithm SVD algorithm} that will optionally
     ...
     *         The returned matrix will also be a {@link edu.ucla.sspace.matrix.SparseMatrix}
     */
    ...
    public static DoubleVector multiplyByTransposedMatrix(SparseDoubleVector sparseVector, Matrix matrix) {
        //System.out.println(matrix.rows() + " x " + matrix.columns());
        DoubleVector result = new DenseVector(matrix.rows()); // due to transposition rows instead of columns!

        for (int i = 0; i < result.length(); i++) {
            double resultValue = 0;
            for (int nz : sparseVector.getNonZeroIndices()) {
                resultValue += sparseVector.get(nz) * matrix.get(i, nz); // due to transposition rows nz and i swapped!
            }
